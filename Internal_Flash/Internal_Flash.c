...
FlashStatus_t FlashInit(void)
{
	...
	if(CoreHCKL > 0 && CoreHCKL <= 24000000)
		__HAL_FLASH_SET_LATENCY(FLASH_LATENCY_0);
	else if(CoreHCKL > 24000000 && CoreHCKL <= 48000000)
		__HAL_FLASH_SET_LATENCY(FLASH_LATENCY_1);
	else if(CoreHCKL > 48000000 && CoreHCKL <= 72000000)
...
FlashStatus_t FlashErasePage(uint32_t Address)
{
	...
	FlashErase_s.TypeErase = FLASH_TYPEERASE_PAGES;
	FlashErase_s.Banks = FLASH_BANK_1;
	FlashErase_s.PageAddress = Address;
	FlashErase_s.NbPages = 1;
...
FlashStatus_t FlashWriteData(uint32_t Address, uint32_t *pData, uint32_t Size)
{
	...
	if((ErrCode = FlashErasePage(Address)) != FLASH_STATUS_OK)
		return ErrCode;
